(function (window) {
  ...
  const DEFAULTS = {
    ...
    iconClasses: {
      toggleRowOpen: "fa-solid fa-chevron-right fa-fw rotate-icon-0",
      toggleRowClose: "fa-solid fa-chevron-right fa-fw rotate-icon-90",
  ...
  function init(userOptions = {}) {
    ...
    // zentrale Sichtbarkeitssteuerung
    function setVisible(tr, visible) {
      tr.dataset.visible = visible ? "true" : "false";
      tr.style.display = visible ? "" : "none";
    }

    // Cache Original-HTML für sauberes Highlighting
    ...
    function performSearch(val) {
      ...
      if (!q) {
        rows.forEach((tr) => {
          ...
          const btn = tr.querySelector(".toggle-btn");
          if (btn) {
            const open = visible && !isRoot; // nur echte Kinder gelten als "sichtbar"; für Root kein open erzwingen
            btn.setAttribute("data-open", open ? "true" : "false");
            btn.classList.toggle("open", open);
            btn.setAttribute("aria-expanded", open ? "true" : "false");
          }
    ...
    function collapseAll() {
      rows.forEach((tr) => {
        if (!isDataRow(tr)) return;
        const visible = getLevel(tr) === 0; // Root sichtbar, Kinder zu
        setVisible(tr, visible);
        const btn = tr.querySelector(".toggle-btn");
        if (btn) {
          const isOpen = false;
